...
│   └── publisher.py        ← Sends reports to Discord ⬜ TODO
│
...
### Phase 2 — Reports & Delivery 🔄 IN PROGRESS
- [x] pdf_generator.py — professional PDF reports
- [ ] publisher.py — Discord delivery via webhook
...
1. Build `discord/publisher.py`
   - Load webhook URLs from .env
   - Send PDF file as attachment to correct channel
   - Include summary embed: top 3 stocks, date, portfolio type
   - Test with a real Discord webhook
...
3. Build `db/database.py` — SQLite connection, schema creation
4. Build `scraper/pse_scraper.py` — PSE Edge stock list and prices
5. Build `scraper/pdf_parser.py` — annual report PDF parser
6. Build `engine/validator.py` — data quality checks

**THEN — Phase 4 automation:**
7. Build `scheduler.py` — weekly automated runs
8. Build `alerts/alert_engine.py` — disclosure alerts

---

## 14. DISCORD SETUP (for publisher.py)
...
1. Embed message with: portfolio name, date, top 3 stocks and scores
2. PDF file attached directly to the message
3. Footer: "PSE Quant SaaS | For research purposes only"
